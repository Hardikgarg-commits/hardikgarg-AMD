...
class Drone:
    ...
    def move(self, heatmap, wind_speed=0, wind_dir="N"):
        ...
        hx = max(0, min(GRID_SIZE - 1, int(self.x)))
        hy = max(0, min(GRID_SIZE - 1, int(self.y)))
        heatmap[hx][hy] += 1
...
class Swarm:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("CUDA reported available but get_device_name failed; falling back to CPU label")
            self.gpu_name = "CPU (CUDA unavailable)"

    # ==================================================
    # DRONE LOOKUP
    # ==================================================

    def has_drone(self, drone_id):
        return any(d.id == drone_id for d in self.drones)
    ...
    def manual_move(self, drone_id, dx, dy):
        ...
        logger.warning("manual_move called with unknown drone_id=%d", drone_id)
    ...
    def set_direction(self, drone_id, direction):
        ...
        logger.warning("set_direction called with unknown drone_id=%d", drone_id)
    ...
    def get_state(self):
        ...
        try:
            ...
            logger.warning("Failed to read system metrics via psutil", exc_info=True)
            cpu_usage = 0.0
            ram_usage = 0.0
